...
std::pair<BinarySearchTree::ConstIterator, BinarySearchTree::ConstIterator> BinarySearchTree::equalRange(
    const Key &key) const
{
...
{
    ConstIterator it    = find(key);
    ConstIterator itMax = ++it;
    while (it->first == key)
    {
        itMax = (itMax->second > it->second ? itMax : it);
        it++;
    }
    return itMax;
}
